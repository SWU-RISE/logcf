...
\rev{{\em Vincent's theorem} provides  a possible method to isolate the real roots. But it needs to divide all $\RR$ to small many intervals and the width of these intervals is smaller than a given  finite value. Hence, it can not   directly use {\em Vincent's theorem} to isolate the real roots. For employing  {\em Vincent's theorem}, we need to discard  the interval which can make sure there is no the real root in it. {\em Algorithm \ref{alg:lb}} provide a lower  bound $lu$ of positive roots of given $p$, then interval  $(0,lb)$ contains no the real roots, so we can safely  discard
