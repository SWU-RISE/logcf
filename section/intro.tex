...
We provide a new method for computing such bounds with time complexity $O(n\log(u+1))$ \rev{addition and multiplication symbolic operators}, where $u$ is the optimal upper bound satisfying Theorem \ref{thm:log}. Besides, compared
...
main algorithm for real root isolation based on  continued fraction. We present some theoretically results about  upper bounds of positive roots in section \ref{sec:thm}.  In section \ref{sec:alg}, we provide a new algorithm for
