...
\begin{theorem}
	\label{thm:log}
  Suppose   $p=a_nx^n+a_{n-1}x^{n-1}+\cdots+a_1x+a_0\ (a_n>0)$  is a univariate polynomial in $x$ with real coefficients.  Then a nonnegative number $u$ is an upper bound of positive roots of $p$ if $u$   satisfies that  $\sum_{i=j}^na_i u^{i-j}\ge 0$ for $j=0,\cdots, n $.
\end{theorem}
\begin{proof}
...
For {\em Example \ref{ex:exp1}},
	...
	\end{itemize}
	 Hence, $  \sum_{i=j}^6 a_i u^{i-j} \ge 0, j=0\cdots,6$ when   $u=-1+\sqrt{5}$. By {\em Theorem \ref{thm:log}}, $-1+\sqrt{5}$ is one  upper bound of positive roots of $p_1$.
}
