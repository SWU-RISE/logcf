...
\rev{For {\em Example \ref{ex:exp1}},
	\begin{itemize}
			\item  $\sum_{i=6}^6 a_i x^{i-j}$ is $1$.
		\item  $\sum_{i=5}^6 a_i x^{i-j}$ is $x+2$ and $x$ is positive in $x\ge0$.  
	...
	\end{itemize}
	 Hence, $\min\{x\mid  \sum_{i=j}^6 a_i x^{i-j} \ge 0, j=0\cdots,6 \} =-1+\sqrt{5}$.  } 
...
\rev{So $p(x)$ can be written as $p(x)=\sum (c_{i_1}x^{e_{i_1}}-b_{i_2}x^{e_{i_2}})+g(x)$ where $c_{i_1}> 0,b_{i_2}>0,e_{i_1}> e_{i_2}, e_{{i+1}_1}\ge e_{i_1} $  and all the coefficients of polynomial $g(x)$ are positive.
